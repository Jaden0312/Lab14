...
//버튼 클릭시
var changeQuote = document.getElementById('another-quote');
var i = -1;
var rgbNumber = Math.floor(Math.random() * 256);
changeQuote.addEventListener('click', () => {
    i += 1;
    console.log(quotes[i].quote);
    console.log(quotes[i].source);//문제점: 버튼 클릭시 1씩 증가하기 때문에 quotes[i] 초과시 더 이상 안나옴
    document.getElementById('quote').innerHTML = quotes[i].quote;
    document.getElementById('source').innerHTML = quotes[i].source;
    //문제점: 버튼 클릭시 1씩 증가하기 때문에 quotes[i] 초과시 더 이상 안나옴
    //다른 논리로 한번 로테가 되도 계속 나오게 하는 방법이 있는지..
    document.getElementById ( "container" ).style.backgroundColor = 'rgb('+rgbNumber+','+rgbNumber+','+rgbNumber+')';
});  
//문제점: addEventListener의 특성상 한번 변경되고 그다음 클릭시 변경 안 됨....
//의문: rgbNumber 앞뒤에 +는 왜 붙는가.....
